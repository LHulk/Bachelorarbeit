...
die einen Punkt $P=(x,y,z)$ in homogenen Koordinaten $\tilde P = (x,y,z,1)$ auf einen Punkt $\tilde C = (wu,wv, w)$ beziehungsweise nach der perspektivischen Division $C = (u,v)$ auf die Bildebene abbildet, wobei die $a_{i,j}$ von den intrinsischen und extrinsischen Parametern der Kamera abhängen \cite{Heikkila1997}.

Wir konstruieren nun schrittweise die Matrix $A$:

 Zunächst wird das Weltkoordinatensystem wie in Abbildung \ref{fig:extrinsic} mit einer Rotation und einer Translation in das Kamerakoordinatensystem überführt. 
...
\todo{möglicherweise Reihenfolge hier ändern, so wie sie auch später benutzt werden}
...
\section{Ausgleichsproblem}
Gegeben seien $m$ Messdaten $(x_1,y_1),\dotsc,(x_m,y_m)$, sowie $n$ Funktionen $u_1,\dotsc,u_n$ mit $n\leq m$ und $n,m\in\mathbb{N}$. Hier Zitat einfügen. Skript von Ohlberger

Gesucht ist eine Linearkombination $u(x) = \sum_{i=1}^{m} c_iu_i(x)$ welche die mittlere Abweichung minimiert, also 

\[
	\Delta_2 = \inf_{c_1,\dotsc,c_n\in\mathbb{R}} \left(\sum_{j=1}^{m}\left(\sum_{i=1}^{n}\left(c_iu_i(x_j)\right) - y_j\right)^2\right)^{\frac{1}{2}}.
\]
...
\begin{aligned}
	...
	A &= (a_{ij})\in\mathbb{R}^{m\times n}\quad\text{mit}\quad a_{ij} = u_i(x_j)
\end{aligned}
\]
...
Die Lösung des Ausgleichproblems kann mit einer Singulärwertzerlegung gelöst werden. Die Matrix $A\in\mathbb{R}^{m\times n}$ kann geschrieben werden als:
 
\[
A = U\Sigma V^T
\]

mit orthogonalen Matrizen $U\in\mathbb{R}^{m\times m}$ und $V\in\mathbb{R}^{n\times n}$ und einer Diagonalmatrix $\Sigma\in\mathbb{R}^{m\times n}$ mit den absteigend sortierten Singulärwerten auf der Diagonale. Setzt man diese Form für $A$ in die Normalengleichung (\ref{eq:normaleq}) ein so ergibt sich:
\begin{equation*}
((U\Sigma V^T)^T\cdot (U\Sigma V^T))^{-1}\cdot (U\Sigma V^T)^T = V\Sigma^{-1}U^T
\end{equation*}
